...
Tilemap.prototype = {
    loadTilemap: function(tilemap, onLoadCallback) {
        ...
        this.tilemapLevels = tilemap.tilemapLevels;
        this.tilesize = Vec2.Vec2(tilemap.tileWidth, tilemap.tileHeight);
        this.tilemaps = {};
        ...
        for(var tilemapLevel in tilemap.tilemaps) {
            ...
            if(tilemap.tilemaps.hasOwnProperty(tilemapLevel)) {
                ...
                this.tilemaps[tilemapLevel] = newTilemap;
            }
        }
    },
    ...
    getImagesAtTilePositions: function(tilePositions, tilemapLevel) {
        ...
        for(var i = 0; i < tilePositions.length; ++i) {
            ...
            var image = this.tilemaps[tilemapLevel][tile.x][tile.y];

            image.renderPosition = Vec2.scale(Vec2.multiply(tile, this.tilesize), tilemapLevel);
            image.scaledSize = Vec2.scale(this.tilesize, tilemapLevel);
            images.push(image);
        }
        return images;
    },
    getSurroundingTilePositions: function(tilePosition, tilemapLevel, radius) {
        ...
        for(var y = negBoundary; y < posBoundary; ++y) {
            for(var x = negBoundary; x < posBoundary; ++x) {
                ...
                var tilemapHeight = this.tilemaps[tilemapLevel][0].length;
                var tilemapWidth  = this.tilemaps[tilemapLevel].length;
    ...
    getTilePosition: function(imagePosition, tilemapLevel) {
        ...
        var tileSizeInImageCoords = Vec2.scale(this.tilesize, tilemapLevel);
